...
function SearchBar({ query }: { query: string }) {
  return (
    <div className="flex items-center rounded-full border border-neutral-200 bg-white px-5 py-3">
      <svg className="mr-4 h-5 w-5 text-neutral-400" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-label="Search">
...
function SerpResultRow({
  name, url, snippet, isBrand, isBlurred, isAtTop, isClicked,
}: {
  name: string; url: string; snippet: string; isBrand?: boolean; isBlurred: boolean; isAtTop?: boolean; isClicked?: boolean;
}) {
  return (
    <motion.div
      ...
      {isBrand && (
        <motion.div
          ...
          animate={{
            ...
            boxShadow: isAtTop ? "0 0 40px rgba(59,130,246,0.3)" : "0 0 20px rgba(59,130,246,0.15)",
          }}
          transition={{ duration: 0.3 }}
        />
      )}
      <div className={`mb-1.5 text-xs ${isBrand ? "font-medium text-neutral-700 dark:text-neutral-300" : "text-neutral-500 dark:text-neutral-400"}`}>{url}</div>
      <motion.div
        className={`text-[15px] ${isBrand ? "font-semibold" : ""}`}
        animate={{ textDecoration: isClicked ? "underline" : "none" }}
        transition={{ duration: 0.15 }}
      >
        <span className={isClicked ? "text-purple-600 dark:text-purple-400" : "text-[#1a0dab] dark:text-blue-300"}>{name}</span>
      </motion.div>
      <div className={`mt-2 text-xs leading-relaxed ${isBrand ? "text-neutral-600 dark:text-neutral-300" : "text-neutral-500 dark:text-neutral-400"}`}>{snippet}</div>
    </motion.div>
  );
}

function MouseCursor({ isVisible, isClicking }: { isVisible: boolean; isClicking: boolean }) {
  return (
    <motion.div
      className="pointer-events-none absolute z-30"
      initial={{ opacity: 0, x: 350, y: 120 }}
      animate={{ opacity: isVisible ? 1 : 0, x: isVisible ? 140 : 350, y: isVisible ? 40 : 120, scale: isClicking ? 0.9 : 1 }}
      transition={{ duration: isVisible ? CURSOR_MOVE_DURATION : 0.08, ease: [0.22, 1, 0.36, 1] }}
    >
      <svg width="28" height="28" viewBox="0 0 24 24" fill="none" aria-label="Cursor">
...
function GoogleSerpGraphic({ onComplete }: { onComplete: () => void }) {
  ...
  const [phase, setPhase] = useState<"idle" | "climbing" | "top" | "cursor" | "click">("idle");

  const competitors = [
    { id: "c1", name: "Competitor 1 — Industry Solutions", url: "competitor1.com", snippet: "Professional services for your business..." },
    { id: "c2", name: "Competitor 2 — Industry Solutions", url: "competitor2.com", snippet: "Professional services for your business..." },
    { id: "c3", name: "Competitor 3 — Industry Solutions", url: "competitor3.com", snippet: "Professional services for your business..." },
    { id: "c4", name: "Competitor 4 — Industry Solutions", url: "competitor4.com", snippet: "Professional services for your business..." },
    { id: "c5", name: "Competitor 5 — Industry Solutions", url: "competitor5.com", snippet: "Professional services for your business..." },
    { id: "c6", name: "Competitor 6 — Industry Solutions", url: "competitor6.com", snippet: "Professional services for your business..." },
    { id: "c7", name: "Competitor 7 — Industry Solutions", url: "competitor7.com", snippet: "Professional services for your business..." },
    { id: "c8", name: "Competitor 8 — Industry Solutions", url: "competitor8.com", snippet: "Professional services for your business..." },
    { id: "c9", name: "Competitor 9 — Industry Solutions", url: "competitor9.com", snippet: "Professional services for your business..." },
  ];
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      await Promise.all([
        brandControls.start({ y: brandEndY, transition: { duration: CLIMB_DURATION, ease: EASE } }),
        competitorControls.start({ y: competitorEndY, transition: { duration: CLIMB_DURATION, ease: EASE } }),
      ]);
      if (cancelled) return;
      setPhase("top");
  ...
  }, [competitorControls, brandControls, competitorStartY, brandStartY, onComplete]);
  ...
  return (
    <div className="flex w-[640px] max-w-[95vw] items-start" style={{ minHeight: GRAPHIC_HEIGHT }}>
      <div className="relative w-full overflow-hidden rounded-3xl border-2 border-neutral-400 bg-white dark:border-neutral-600 dark:bg-neutral-800">
        <div className="border-neutral-100 border-b bg-white px-6 pt-6 pb-4 dark:border-neutral-700 dark:bg-neutral-800">
          <GoogleLogo />
          <div className="mt-5"><SearchBar query="best product for my needs" /></div>
          <div className="mt-5 flex items-center gap-6 text-neutral-500 text-sm">
            <span className="-mb-px border-[#4285f4] border-b-[3px] pb-3 font-medium text-neutral-800">All</span>
            <span className="pb-3">Images</span>
            <span className="pb-3">Videos</span>
            <span className="pb-3">News</span>
          </div>
        </div>
        <div className="relative overflow-hidden bg-white dark:bg-neutral-800" style={{ height: VIEWPORT_H }}>
          <motion.div className="absolute inset-x-0 top-0" animate={competitorControls} initial={{ y: competitorStartY }} style={{ willChange: "transform" }}>
            {competitors.map((c) => (
              <SerpResultRow key={c.id} name={c.name} url={c.url} snippet={c.snippet} isBrand={false} isBlurred={isClimbing} />
            ))}
          </motion.div>
          <motion.div className="absolute inset-x-0 top-0 z-10" animate={brandControls} initial={{ y: brandStartY }} style={{ willChange: "transform" }}>
            <SerpResultRow name="Your Brand — The Best Solution" url="yourbrand.com" snippet="Discover why customers choose us..." isBrand={true} isBlurred={false} isAtTop={isAtTop} isClicked={isClicked} />
          </motion.div>
          <MouseCursor isVisible={showCursor} isClicking={isClicked} />
        </div>
      </div>
    </div>
  );
...
function GeminiSparkle() {
  return (
    <svg width="20" height="20" viewBox="0 0 28 28" fill="none" aria-label="AI Overview">
...
function AIOverviewGraphic({ onComplete }: { onComplete: () => void }) {
  const [phase, setPhase] = useState<"idle" | "thinking" | "typing" | "highlight" | "done">("idle");
  ...
  const showText = phase === "typing" || phase === "highlight" || phase === "done";
    ...
    <div className="flex w-[640px] max-w-[95vw] items-start" style={{ minHeight: GRAPHIC_HEIGHT }}>
      <div className="relative w-full overflow-hidden rounded-3xl border-2 border-neutral-400 bg-white dark:border-neutral-600 dark:bg-neutral-800">
        ...
        <div className="flex items-center gap-2.5 border-neutral-100 border-b px-6 pt-5 pb-4 dark:border-neutral-700 dark:bg-neutral-800">
          <GeminiSparkle />
          <span className="font-medium text-[15px] text-neutral-800 dark:text-neutral-200">AI Overview</span>
          <div className="ml-auto">
            <svg className="h-5 w-5 text-neutral-400" fill="currentColor" viewBox="0 0 20 20" aria-label="More">
          ...
          <AnimatePresence>
            {isThinking && (
              ...
              >
                ...
                <span className="text-[14px] text-neutral-500 dark:text-neutral-400">Thinking...</span>
              </motion.div>
            )}
          </AnimatePresence>
              ...
              >
                <p className="mb-4 text-[15px] text-neutral-700 leading-relaxed dark:text-neutral-300">
                  ...
                  <motion.span
                    ...
                    animate={{
                      ...
                      backgroundColor: highlightBrand ? "rgba(59, 130, 246, 0.1)" : "transparent",
                    }}
                ...
                <AnimatePresence>
                  {highlightBrand && (
                    ...
                    >
                      <svg className="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-label="Source">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M13.828 10.172a4 4 0 00-5.656 0l-4 4a4 4 0 105.656 5.656l1.102-1.101m-.758-4.899a4 4 0 005.656 0l4-4a4 4 0 00-5.656-5.656l-1.1 1.1" />
                      </svg>
                      <span className="cursor-pointer text-blue-600 hover:underline dark:text-blue-400">yourbrand.com</span>
                    </motion.div>
                  )}
                </AnimatePresence>
              </motion.div>
...
function ChatGPTLogo() {
  return (
    <svg width="24" height="24" viewBox="0 0 24 24" fill="none" className="text-neutral-900 dark:text-white" aria-label="ChatGPT">
...
function ChatGPTGraphic({ onComplete }: { onComplete: () => void }) {
  const [phase, setPhase] = useState<"idle" | "typing" | "show" | "sources" | "done">("idle");
  ...
  const showResponse = phase === "show" || phase === "sources" || phase === "done";
    ...
    <div className="flex w-[640px] max-w-[95vw] items-start" style={{ minHeight: GRAPHIC_HEIGHT }}>
      <div className="relative w-full overflow-hidden rounded-3xl border-2 border-neutral-400 bg-white dark:border-neutral-600 dark:bg-neutral-800">
        ...
        <div className="flex items-center gap-2.5 border-neutral-100 border-b px-6 pt-5 pb-4 dark:border-neutral-700">
          <ChatGPTLogo />
          <span className="font-medium text-[15px] text-neutral-800 dark:text-neutral-200">ChatGPT</span>
        </div>
        ...
        <div className="min-h-[220px] px-6 py-5">
          ...
          <div className="mb-5 flex justify-end">
            <div className="rounded-2xl bg-neutral-100 px-4 py-2.5 dark:bg-neutral-700">
              <p className="text-[14px] text-neutral-800 dark:text-neutral-200">best product for my needs</p>
            </div>
          </div>
            ...
            {phase === "typing" && (
              <div className="flex items-center gap-2 py-2">
                ...
                <div className="flex gap-1">
                  <motion.div animate={{ opacity: [0.4, 1, 0.4] }} transition={{ duration: 1, repeat: Infinity }} className="h-1.5 w-1.5 rounded-full bg-neutral-400" />
                  <motion.div animate={{ opacity: [0.4, 1, 0.4] }} transition={{ duration: 1, repeat: Infinity, delay: 0.2 }} className="h-1.5 w-1.5 rounded-full bg-neutral-400" />
                  <motion.div animate={{ opacity: [0.4, 1, 0.4] }} transition={{ duration: 1, repeat: Infinity, delay: 0.4 }} className="h-1.5 w-1.5 rounded-full bg-neutral-400" />
                </div>
              </div>
            )}
                  ...
                  <p className="mb-3 text-[15px] text-neutral-700 leading-relaxed dark:text-neutral-300">
                    ...
                    <motion.span
                      ...
                      animate={{
                        ...
                        backgroundColor: showSources ? "rgba(59, 130, 246, 0.1)" : "transparent",
                      }}
                      style={{ padding: "2px 6px", borderRadius: "4px" }}
                      transition={{ duration: 0.3 }}
                    >
                    ...
                    {showSources && (
                      ...
                      >
                        ...
                        <div className="inline-flex items-center gap-2 rounded-lg border border-neutral-200 bg-neutral-50 px-3 py-2 dark:border-neutral-600 dark:bg-neutral-700">
                          <svg className="h-4 w-4 text-neutral-400" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-label="Source">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M13.828 10.172a4 4 0 00-5.656 0l-4 4a4 4 0 105.656 5.656l1.102-1.101m-.758-4.899a4 4 0 005.656 0l4-4a4 4 0 00-5.656-5.656l-1.1 1.1" />
                          </svg>
                          <span className="cursor-pointer text-[13px] text-blue-600 hover:underline dark:text-blue-400">yourbrand.com</span>
                          <svg className="h-4 w-4 text-green-500 dark:text-green-400" fill="currentColor" viewBox="0 0 20 20" aria-label="Verified">
        ...
        <div className="px-4 pb-4">
          <div className="flex items-center gap-3 rounded-2xl border border-neutral-200 bg-neutral-50 px-4 py-3 dark:border-neutral-600 dark:bg-neutral-700">
            <svg className="h-5 w-5 text-neutral-400" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-label="Add">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4v16m8-8H4" />
            </svg>
            <span className="flex-1 text-[14px] text-neutral-400">Message ChatGPT</span>
            <svg className="h-5 w-5 text-neutral-400" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-label="Send">
  ...
  return (
    <section className="relative min-h-[85vh] overflow-hidden pt-24 pb-12 sm:min-h-[95vh] sm:py-20">
      ...
      <div className="pointer-events-none absolute inset-0">
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_30%_20%,rgba(16,185,129,0.06)_0%,transparent_50%)] dark:bg-[radial-gradient(circle_at_30%_20%,rgba(16,185,129,0.14)_0%,transparent_50%)]" aria-hidden />
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_70%_80%,rgba(16,185,129,0.04)_0%,transparent_50%)] dark:bg-[radial-gradient(circle_at_70%_80%,rgba(16,185,129,0.1)_0%,transparent_50%)]" aria-hidden />
      </div>
      <div className="relative mx-auto max-w-6xl px-4 sm:px-6">
        <h1 className="mb-4 text-center sm:mb-6">
          ...
          <span className="mt-1 block font-light font-serif text-[32px] text-neutral-900 italic leading-[1.1] tracking-[-0.02em] sm:mt-2 sm:text-[48px] md:text-[64px] lg:text-[80px] dark:text-white">
            Top of Search<span className="text-emerald-600 dark:text-emerald-400">.</span>
          </span>
        </h1>
        ...
        <div className="mb-8 text-center sm:mb-12">
          <p className="mx-auto mb-6 max-w-2xl text-base text-neutral-600 sm:text-lg dark:text-neutral-400">
            Fluid Posts is the end-to-end SEO/GEO tool that automates organic rankings<span className="text-emerald-600 dark:text-emerald-400">.</span>
          </p>
          <WaitListDialog
            trigger={
              <button
        ...
        <div className="flex justify-center overflow-hidden" style={{ height: GRAPHIC_HEIGHT }}>
          <AnimatePresence mode="wait">
            ...
            >
              {CurrentComponent ? <CurrentComponent onComplete={handleComplete} /> : null}
            </motion.div>
          </AnimatePresence>
        </div>
          ...
          <div className="inline-flex items-center gap-1 rounded-full border-2 border-neutral-300 bg-white p-1.5 dark:border-neutral-600 dark:bg-neutral-800">
            {GRAPHICS.map((g) => (
              <button
                key={g.key}
                type="button"
                onClick={() => { const idx = GRAPHICS.findIndex(x => x.key === g.key); if (idx >= 0) setCurrentGraphic(idx); }}
