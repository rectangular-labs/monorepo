    ...
    <Section className="border-t border-border">
      <div className="mx-auto max-w-6xl gap-10 lg:grid lg:grid-cols-[5fr,4fr] lg:items-center">
        <div className="space-y-4">
          <p className="text-xs uppercase tracking-[0.4em] text-muted-foreground">You stay in control</p>
        ...
        <div className="rounded-3xl border border-border/40 bg-muted/60 p-6 shadow-sm">
          <p className="text-sm font-semibold text-foreground">Live checkpoints</p>
          <ul className="mt-4 space-y-3">
            {checkpoints.map((item) => (
              <li
                key={item}
                className="flex items-center justify-between rounded-2xl border border-border/60 bg-background/80 px-4 py-3 text-sm text-foreground/80"
              >
                <span>{item}</span>
          ...
          <p className="mt-4 text-xs uppercase tracking-[0.4em] text-muted-foreground">Instant iteration</p>
        </div>
      </div>
    </Section>
  );
