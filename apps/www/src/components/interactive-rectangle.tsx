...
function HudOverlay({
  ...
  setAttitude: React.Dispatch<
    React.SetStateAction<{ p: number; y: number; r: number }>
  >;
  setControl: React.Dispatch<React.SetStateAction<{ x: number; y: number }>>;
  setOffsetDeg: React.Dispatch<
    React.SetStateAction<{ px: number; yy: number }>
  >;
...
export function InteractiveRectangle() {
  ...
  return (
    <div className="h-72 w-full md:relative md:h-80">
      <div className="pointer-none absolute inset-0 select-none font-mono text-[10px] tracking-wider md:text-xs">
        <div className="absolute top-3 left-3 text-cyan-700 dark:text-cyan-300/90">
          ...
          <div className="text-cyan-900 dark:text-cyan-100">
            P {signed(attitude.p)}°
            <span className="mx-2">Y {signed(attitude.y)}°</span>R{" "}
            {signed(attitude.r)}°
          </div>
        </div>
        ...
        <div className="absolute top-3 right-3 text-lime-700 dark:text-lime-200/90">
          ...
          <div className="flex items-center gap-2 text-lime-900 dark:text-lime-100">
            ...
            {peakFlash ? (
              <span className="rounded-sm bg-lime-500/15 px-1 py-0.5 text-[9px] text-lime-700 uppercase tracking-widest dark:bg-lime-300/20 dark:text-lime-200">
                PEAK
              </span>
            ) : null}
          </div>
