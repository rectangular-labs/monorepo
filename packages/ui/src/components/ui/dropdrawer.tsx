...
function DropDrawerItem({
  ...
  if (isMobile) {
    ...
    const content = (
      // biome-ignore lint/a11y/noStaticElementInteractions: from drop drawer
      // biome-ignore lint/a11y/useKeyWithClickEvents: from drop drawer
      <div
        aria-disabled={disabled}
        className={cn(
          "flex cursor-pointer items-center gap-2 px-4 py-4",
...
function DropDrawerGroup({
    ...
    return filteredChildren.flatMap((child, index) => {
      ...
      return [
        ...
        <div
          ...
          key={`separator-${
            // biome-ignore lint/suspicious/noArrayIndexKey: from drop drawer
            index
          }`}
        />,
      ];
  ...
  if (isMobile) {
    return (
      // biome-ignore lint/a11y/useSemanticElements: from drop drawer
      <div
        className={cn(
