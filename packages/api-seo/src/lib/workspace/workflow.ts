...
function analyzeHeroAndH2Images(args: {
  ...
  if (heroImageIndex == null && heroCandidates.length === 1) {
    // biome-ignore lint/style/noNonNullAssertion: AI slop
    heroImageIndex = heroCandidates[0]!.index;
    heroImageDetection = "before-first-h2";
  }

  if (heroImageIndex == null && args.images.length > 0) {
    // biome-ignore lint/style/noNonNullAssertion: AI slop
    heroImageIndex = args.images[0]!.index;
  ...
  if (h2Indices.length > 0) {
    for (let i = 0; i < h2Indices.length; i++) {
      ...
      const end =
        // biome-ignore lint/style/noNonNullAssertion: AI slop
        i + 1 < h2Indices.length ? h2Indices[i + 1]! : args.markdown.length;
...
export function analyzeArticleMarkdownForReview(args: {
  ...
  return {
    ...
    imagesTotal: heroImageCount + nonHeroH2ImageCount,
    ...
    nonHeroH2ImageCount,
    h2SectionsWithAnyImageCount,
    h2Headings: h2Headings.slice(0, 80),
    outlineHeadingsExpected: outlineHeadingsExpected.slice(0, 80),
    missingOutlineHeadings: missingOutlineHeadings.slice(0, 40),
    emDashCount,
    emDashContexts: extractEmDashContexts(args.markdown),
    tableBlockCount: countMarkdownTables(args.markdown),
    bulletLineCount,
    bulletLinesWithBoldHeadingAndColonCount,
    bulletFormatExamples: examples,
    imagesMissingAltTextCount: images.filter((img) => !img.alt.trim()).length,
