...
function createClient(config: ShopifyConfig) {
  ...
  return {
    ...
    async exchangeCodeForToken(params: {
      ...
      try {
        ...
        return ok({ accessToken: data.access_token });
      } catch (error) {
    ...
    async healthCheck(config: ShopifyConfig) {
      const result = await this.listBlogs(config);
      if (!result.ok) return result;
      return ok({ ok: true });
    },

    async publish(config: ShopifyConfig, content: ContentPayload) {
      if (!config.blogId) {
